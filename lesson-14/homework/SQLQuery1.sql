...
----1. Ism ustunini vergul bilan ikkita alohida ustunga bo'lish uchun SQL so'rovini yozing: Ism va Familiya.(TestMultipleColumns)
...
--- 2. Jadvaldagi satrlarni topish uchun SQL so'rovini yozing, unda satrning o'zi % belgisini o'z ichiga oladi.(TestPercent)


SELECT * FROM TestPercent WHERE Strs LIKE '%\%%' ESCAPE '\';

---- 3. Bu boshqotirmada siz nuqta (.) ga asoslangan qatorni ajratishingiz kerak bo'ladi.(Splitter)
...
---6. Satrdagi bo'shliqlarni hisoblash uchun SQL so'rovini yozing.(CountSpaces)
...
----7. Menejerlaridan ko'proq maosh oladigan xodimlarni aniqlaydigan SQL so'rovini yozing.(Xodim)
...
----8. Kompaniyada 10 yildan ortiq, lekin 15 yildan kam ishlagan xodimlarni toping. Xodimning identifikatori, ismi, familiyasi, ishga qabul qilingan sanasi va ishlagan yillarini ko‘rsating (joriy sana va ishga qabul qilingan sana o‘rtasidagi yillar soni sifatida hisoblanadi).(Xodimlar)
...
----1. Butun son qiymatlari va belgilar qiymatlarini ikki xil ustunga ajratish uchun SQL so‘rovini yozing.(rtcfvty34redt)
...
----3. Har bir oʻyinchi uchun birinchi kirish sanasi haqida xabar beruvchi SQL soʻrovini yozing.(Faoliyat)
...
---4. Sizning vazifangiz ushbu ro'yxatdagi uchinchi elementni qaytarishdir.(mevalar)
...
---- 5. Satrdagi har bir belgi qatorga aylantiriladigan jadval yaratish uchun SQL so'rovini yozing.(sdgfhsdgfhs@121313131)





--- 6. Sizga ikkita jadval beriladi: p1 va p2. Ushbu jadvallarni id ustuniga qo'shing. Tushunish: p1.code qiymati 0 bo'lsa, uni p2.code qiymati bilan almashtiring.(p1,p2)
...
----7. Har bir xodimning HIRE_DATE sanasi asosida bandlik bosqichini aniqlash uchun SQL soʻrovini yozing. Bosqichlar quyidagicha belgilanadi:
...
---- 8. Vals.(GetIntegers) nomli ustunda satr boshida paydo boʻladigan butun son qiymatini chiqarish uchun SQL soʻrovini yozing.



--Difficult Tasks

---- 1. Bu boshqotirmada siz vergul bilan ajratilgan qatorning birinchi ikki harfini almashtirishingiz kerak.(MultipleVals)
...
---- 2. Har bir oʻyinchi uchun birinchi kirgan qurilma haqida xabar beruvchi SQL soʻrovini yozing.(Faoliyat)
...
WITH WeeklySales AS (
    SELECT 
        ...
        FinancialWeek,
        Area,
        SUM(SalesLocal + SalesRemote) AS TotalSales
    FROM WeekPercentagePuzzle
    GROUP BY FinancialYear, FinancialWeek, Area
),
WeeklyTotals AS (
    SELECT 
        FinancialYear,
...
SELECT 
    ...
    w.TotalSales,
    t.WeekTotal,
    CAST(w.TotalSales AS FLOAT) / t.WeekTotal * 100 AS Percentage
FROM WeeklySales w
JOIN WeeklyTotals t ON w.FinancialYear = t.FinancialYear 
                   AND w.FinancialWeek = t.FinancialWeek
ORDER BY w.FinancialYear, w.FinancialWeek, w.Area;